...
class BaseOptions():
    ...
    def initialize(self):
        self.parser.add_argument('--dataroot', default="./datasets/bolbbalgan4", help='path to images (should have subfolders trainA, trainB, valA, valB, etc)')
        self.parser.add_argument('--batchSize', type=int, default=1, help='input batch size')
        self.parser.add_argument('--loadSize', type=int, default=1, help='scale images to this size')
        ...
        self.parser.add_argument('--max_dataset_size', type=int, default=int(5000), help='Maximum number of samples allowed per dataset. If the dataset directory contains more than max_dataset_size, only a subset is loaded.')
        ...
        self.parser.add_argument('--name', type=str, default='set_exp_name', help='name of the experiment. It decides where to store samples and models')
    ...
    def parse(self):
        ...
        #rename the exp according to the used model
        self.opt.name = "G_{g}_D_{d}_data_{data}".format(g=self.opt.which_model_netG,d=self.opt.which_model_netD,data=self.opt.dataroot.split('/')[-1])
